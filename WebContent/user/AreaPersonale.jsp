...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="model.Cart, java.util.*, model.FilmBean, model.UserBean"%>
<%
...
<body>
	...
	<section id="DataUser">
		<h1 id="intestazione"><%=utente.getName().toUpperCase()+" "+utente.getSurname().toUpperCase()+"-IL TUO PROFILO"%></h1>
		<fieldset id="dati">
			<legend>I tuoi dati</legend>
			<label for="nomeU"><b>Nome</b></label><br>
	    	<i name="nomeU"><%=utente.getName() %></i><br>
	    	
	    	<label for="cognomeU"><b>Cognome</b></label><br>
	    	<i name="cognomeU"><%=utente.getSurname() %></i><br>
	    	
	    	<label for="nascitaU"><b>Data di nascita</b></label><br>
	    	<i name="nascitaU"><%=utente.getBirth() %></i><br>
	    	
	    	<label for="emailU"><b>Email</b></label><br>
	    	<i name="emailU"><%=utente.getEmail() %></i><br>
	    	
	    	<label for="numPrenotazioni"><b>Prenotazioni effettuate</b></label><br>
	    	<i name="numPrenotazioni"><%=utente.getAccessNumber() %></i><br>
		...
		<fieldset id="datiAggiornati">
			...
			<form action="<%=response.encodeURL("UserControl?action=update")%>" method="post" onSubmit="return formValidationRegistration();">
			    ...
			    <span id="emailSpan" style="display:none">Formato email non valida</span><br>
			    ...
			    <span id="pswSpan" style="display:none">Formato password non valida</span><br>
			    ...
			    <span id="pswRepeatSpan" style="display:none">Formato password non valida</span><br>
			...
			if(!showCart.isEmpty()){
				for(ShowBean sh: showCart){
					...
					<tr>
						...
						<td class="delete">
							<form action="<%=response.encodeURL("SpettacoliControl?action=delete")%>" method="post">
